...
def apply_transform(image, scale, rotation, translation_x, translation_y, flip_horizontal):
    ...
    # 4. 添加平移变换
    combined_matrix[0, 2] += translation_x
    combined_matrix[1, 2] += translation_y
    
    # 5. 水平翻转
    if flip_horizontal:
        flip_matrix = np.float32([[-1, 0, w], [0, 1, 0]])
        combined_matrix = np.matmul(flip_matrix, combined_matrix)
    
    # 6. 应用仿射变换
    combined_matrix=combined_matrix[:2, :].astype(np.float32)
